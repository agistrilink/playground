...
1){j.splice(W-1,2);W-=2}j=j.join("/")}return j};y.splitPrefix=function(j,k,s){var q=j.indexOf("!"),u=null;if(q!==-1){u=j.substring(0,q);j=j.substring(q+1,j.length)}j=y.normalizeName(j,k,s);return{prefix:u,name:j,fullName:u?u+"!"+j:j}};y.nameToUrl=function(j,k,s,q){var u,C,F,K;K=x.contexts[s];s=K.config;j=y.normalizeName(j,q,K);if(y.jsExtRegExp.test(j))j+=k?k:"";else{u=s.paths;C=s.packages;q=j.split("/");for(K=q.length;K>0;K--){F=q.slice(0,K).join("/");if(u[F]){q.splice(0,K,u[F]);break}else if(F=C[F]){u=
F.location+"/"+F.lib;if(j===F.name)u+="/"+F.main;q.splice(0,K,u);break}}j=q.join("/")+(k||".js");j=(j.charAt(0)==="/"||j.match(/^\w+:/)?"":s.baseUrl)+j}return s.urlArgs?j+((j.indexOf("?")===-1?"?":"&")+s.urlArgs):j};y.blockCheckLoaded=true;y.checkLoaded=function(j){var k=x.contexts[j||x.ctxName],s=k.config.waitSeconds*1E3,q=s&&k.startTime+s<(new Date).getTime(),u,C=k.defined,F=k.modifiers,K="",aa=false,ba=false,U,ha=x.plugins.isWaiting,ua=x.plugins.orderDeps;if(!k.isCheckLoaded){if(k.config.priorityWait)if(B(k))D(k);
...
z.body;a&&!bb.test(a.nodeName)&&c.css(a,"position")==="static";)a=a.offsetParent;return a})}});c.each(["Left","Top"],function(a,b){var d="scroll"+b;c.fn[d]=function(e){var f=this[0],h;if(!f)return null;return e!==H?this.each(function(){if(h=va(this))h.scrollTo(!a?e:c(h).scrollLeft(),a?e:c(h).scrollTop());else this[d]=e}):(h=va(f))?"pageXOffset"in h?h[a?"pageYOffset":"pageXOffset"]:c.support.boxModel&&h.document.documentElement[d]||h.document.body[d]:f[d]}});c.each(["Height","Width"],function(a,b){var d=
